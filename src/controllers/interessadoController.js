...
async function buscar(req, res) {
    try {
        const linhas = await PRISMACLIENT.interessados.findMany({
            orderBy: {
                id: "asc"
            }
        });
...
async function criar(req, res) {
    try {
        let dados = req.body;
        const linha = await PRISMACLIENT.interessados.create({
            data: dados
        });
...
async function editar(req, res) {
    try {
        let dados = req.body;
        const linha = await PRISMACLIENT.interessados.update({
            data: dados,
            where: {
...
async function deletar(req, res) {
    try {
        ...
        const existe = await PRISMACLIENT.interessados.count({
            where: {
                id: Number(req.params.id)
            }
        })
